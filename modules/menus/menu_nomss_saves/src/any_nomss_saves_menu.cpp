...
KEEP_FUNC AnyNoMssSavesMenu::AnyNoMssSavesMenu(Cursor& cursor)
    : Menu(cursor), lines{
                        ...
                        {"gossip stone trigger skip", FF1, "at start of ff1, about to skip the gossip stone trigger"},
                        {"ff1 interior", FF1_INTERIOR, "ff1 in the rope swing room"},
                        {"windfall", WINDFALL, "at windfall island, just finished talking to korl"},
                        {"forest haven swim", FH_SWIM, "at dri, about to swim to forest haven"},
                        {"early leaf hover", EARLY_LEAF, "outside forest haven for hover to higher entrance"},
                        {"deku tree cutscene skip", DTCS, "inside forest haven at the top, for dtcs"},
                        {"quiver hole in one", HOLE_IN_ONE, "at forest haven, about to swim to thorned fairy island"},
                        ...
                        {"tuner hover to ganon", TUNER_HOVER, "at the bottom of ganondorf\'s bedroom for tuner hover"},
...
void AnyNoMssSavesMenu::draw() {
    ...
    if (GZ_getButtonTrig(SELECTION_BUTTON)) {
        special AnyNoMssSpecials[] = {
            ...
            special(FH_SWIM, SaveMngSpecial_BombsSwim_FHSwim_NoMSS),
            special(EARLY_LEAF, SaveMngSpecial_EarlyLeafHover),
            ...
            special(TUNER_HOVER, SaveMngSpecial_GanonHover),
