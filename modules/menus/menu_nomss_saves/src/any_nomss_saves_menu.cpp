...
KEEP_FUNC AnyNoMssSavesMenu::AnyNoMssSavesMenu(Cursor& cursor)
    : Menu(cursor), lines{
                        ...
                        {"forsaken fortress 1 [alt]", FF1,
                         "at start of ff1 after gossip trigger (no early light cycle)"},
                        ...
                        {"deku tree chin cutscene skip [alt]", DTCS, "inside forest haven at bottom, for normal dtcs"},
                        {"leaf hover [alt]", LEAF_HOVER, "after normal cutscene skip for inside hover"},
                        ...
                        {"tuner hover to ganon [alt]", GANON_HOVER,
...
void AnyNoMssSavesMenu::draw() {
    ...
    if (GZ_getButtonTrig(SELECTION_BUTTON)) {
        special AnyNoMssSpecials[] = {
            ...
            special(O_FH, SaveMngSpecial_Outside_FH_DC),          special(DTCS, SaveMngSpecial_DTCS),
