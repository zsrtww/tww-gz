...
QuestStatusMenu* l_QuestStatusMenu;
...
void onLoad() {
    l_QuestStatusMenu = new QuestStatusMenu(*g_menuMgr->getPermanentData<Cursor>());
    g_drawListener->addListener(onDraw);
}

void onDraw() {
    l_QuestStatusMenu->draw();
}

void onUnload() {
    g_drawListener->removeListener(onDraw);
    delete l_QuestStatusMenu;
}
